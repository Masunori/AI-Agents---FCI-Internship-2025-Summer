    ...
    if st.button("Generate Report", type="primary", use_container_width=True):
        ...
        try:
            ...
            with redirect_stdout(f):
                article_dicts = scrape_articles(
                    parallel=parallel_scraping, max_workers=max_workers
                )
            ...
            with redirect_stdout(f):
                ...
                config_module.GuardrailsConfig.MAX_DOCUMENTS_TO_LLM = (
                    max_documents_to_llm
                )

                final_state = workflow_execution(
                    papers=papers, articles=articles, output_folder_md=output_folder
                )
            ...
            status_text.success(
                f"Report generated successfully in {total_time:.2f} seconds!"
            )
...
with tab2:
    ...
    if report_files:
        ...
        for report_file in report_files:
            ...
            with st.expander(
                f"{file_name} - {file_time.strftime('%Y-%m-%d %H:%M:%S')}"
            ):
        ...
        st.warning(
            "No reports found. Generate your first report in the 'Generate Report' tab!"
        )
